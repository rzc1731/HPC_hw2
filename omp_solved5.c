...
#pragma omp parallel shared(a, b, nthreads, locka, lockb) private(tid)
  ...
  // The bug is caused by deadlock that the first thread tries to get 'lockb'
  // while holding 'locka', and the second thread tries to get 'locka' while
  // holding 'lockb'. Forcing each thread to release its lock before acquiring
  // the other lock solves the deadlock.
